...
const writeCookie = (k, v) => {
	document.cookie = `${k}=${v}; max-age=604800; samesite=strict; secure`; // Default cookie age is a week.
};
...
const handleImgUpload = e => {
	...
	img.onload = e => {
		...
		for (let idx = 0; idx < N * N * 4; idx += 4) {
			...
			const j = (idx / 4) % N;
			const c = [loadedImgArr[idx], loadedImgArr[idx + 1], loadedImgArr[idx + 2]];
			drawCell(i, j, c, actionID);
		}
	};
...
const uploadHelper = file => {
	if (gridList[gridPtr].isEmpty() || confirm("Are you sure you want to upload a file? (Doing so will erase your current drawing.)")) {
		...
		if (file.type && file.type.startsWith("text")) {
			...
			reader.onload = e => handleImgUpload;
